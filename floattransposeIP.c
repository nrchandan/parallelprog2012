...
float* allocateMatrix(int length);
float* generateMatrix(int length, int noinit);
float* loadMatrix(char *infile, int length);
...
void transpose1Tiled(float *matrix, int dimension_N, int size_n, int tilesize_s);
// two tiled transpose of matrix (nxn) with tile size as s1 and s2.
void transpose2Tiled(float *matrix, int dimension_n, int tile1size_s1, int tile2size_s2);

void transposeCacheOblivious(float *matrix, int length_N, int tilesize_n, int row_i, int column_j);
// matrices can be sub-matrices 
void swapTiles(float *matrix1, float *matrix2, int tilesize_s, int length_n);

void printm(float *matrix, int length);
void printmf(float *matrix, int length, const char *filename);

float* M(float *matrix, int dimension, int i, int j);
 ...
 *
 */
void transpose(float *m, int s, int n) 
{
    int i, j, temp;
...
void swapTiles(float *a, float *b, int s, int n)
{
    int i, j, temp;
    for (i=0; i<s; i++) {
        for (j=0; j<s; j++) {
 ...
 *
 */
void printmf(float *m, int n, const char *filename)
{
    int i, j;
    ...
    for (i=0; i<n; i++) {
        for (j=0; j<n; j++) {
//            fprintf(fp, "%d ", m[i][j]);
            fprintf(fp, "%f ", *M(m,n,i,j));
        }
        fprintf(fp, "\n");
    }
}

float* generateMatrix(int n, int noinit)
{
    int i, j;
    float *m;
    m = allocateMatrix(n);
...
float* allocateMatrix(int length)
{
    ...
    matrix = malloc(sizeof(float)*length*length);
    if (matrix==NULL) {
